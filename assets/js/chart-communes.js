function createChartCommune(element) {
    $.ajax({
        ...
        success: function(json) {
            ...
            let lesLibCommunes = [];
            let lesQteArbreCommune = [];

            for (let i = 0; i < json.length; i++) {

                lesLibCommunes[i] = json[i].commune;
                lesQteArbreCommune[i] = json[i].qteArbreCommune;
            }

            // creation du graph ici
            initGraph(element, lesLibCommunes, lesQteArbreCommune, titre);
...
function initGraph(element, labels, data, titre) {
    ...
    let chartCommunes = new Chart(myChart, {
    ...
    console.log('chartCommunes: ' + chartCommunes.options.titre.text);
}