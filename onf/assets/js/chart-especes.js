function createChartEspece(element) {
    $.ajax({
        ...
        success: function(json) {
            ...
            let lesLibEspeces = [];
            let lesQteArbreEspece = [];

            for (let i = 0; i < json.length; i++) {

                lesLibEspeces[i] = json[i].libelleEspece;
                lesQteArbreEspece[i] = json[i].qteArbreEspece;
            }

            // creation du graph ici
            initGraph(element, lesLibEspeces, lesQteArbreEspece, titre);
...
function initGraph(element, labels, data, titre) {
    ...
    let chartEspeces = new Chart(myChart, {
    ...
    console.log('chartEspeces: ' + chartEspeces.options.titre.text);
}